...
class python(Formula):
    ...
    def build(self):
        ...
        files = FileSet()
        files.add(["Include/*"], "include/python2.7", category="dev")
        files.add(["Lib"], "bin", category="rel")
        files.add(["PC/pyconfig.h"], "include/python2.7", category="dev")
        
        os.chdir("PCbuild")
        if self.context.arch == "x64":
            os.chdir("amd64")
        
        files.add(["python27.lib", "python27_d.lib"], "lib", category="dev")
        files.add(["python.exe", "*[!d].dll"], "bin", category="rel")
        files.add(["*[!d].pyd"], "bin/DLLs", category="rel")
        files.add(["python_d.exe", "*_d.dll"], "bin", category="dbg")
        files.add(["*_d.pyd"], "bin/DLLs", category="dbg")
